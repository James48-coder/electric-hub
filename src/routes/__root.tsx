  ...
  return (
    <ThemeProvider defaultTheme="dark">
      <div className="flex min-h-screen w-full bg-background text-foreground selection:bg-primary selection:text-primary-foreground relative">
        ...
        {/* ========================================= */}
        {/* МОБИЛЬНОЕ МЕНЮ (ЖЕСТКО НЕПРОЗРАЧНЫЙ ФОН) */}
        {/* ========================================= */}
        {isMobileMenuOpen && (
          <div className="md:hidden fixed inset-0 z-[100] bg-white dark:bg-[#09090b] flex flex-col animate-in slide-in-from-right-full duration-300">
            <div className="h-16 border-b border-border flex items-center justify-between px-4 shrink-0 shadow-sm bg-white dark:bg-[#09090b]">
              ...
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 rounded-lg bg-neutral-100 dark:bg-neutral-800 text-foreground border border-border shadow-sm">
                <X className="h-5 w-5" />
              </button>
            </div>
            
            <div className="flex flex-col p-4 gap-1 overflow-y-auto pb-24 h-full bg-white dark:bg-[#09090b]">
        ...
        <div className="flex flex-1 flex-col overflow-hidden w-full relative">
          ...
          {/* ПЛАШКА ДЛЯ МОБИЛЬНЫХ: ТОЛЬКО БУРГЕР И ЛОГОТИП */}
          ...
          {/* РОДНАЯ ШАПКА ПОЛЬЗОВАТЕЛЯ (ТЕПЕРЬ ВИДНА И НА МОБИЛЬНЫХ!) */}
          {/* В ней находится твой рабочий переключатель тем */}
          <Header />
