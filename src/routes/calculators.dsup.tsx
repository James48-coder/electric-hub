    ...
    <div className="container mx-auto p-6 max-w-4xl animate-in fade-in duration-500 text-foreground pb-24">
      ...
      <div className="bg-card border border-border rounded-[var(--radius)] shadow-sm overflow-hidden">
        <div className="border-b border-border p-6 bg-primary/5 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">Проектирование ДСУП</h1>
            <p className="text-sm text-muted-foreground mt-1">Выбор сечения проводников для КУП во влажных помещениях (ПУЭ гл. 1.7)</p>
          </div>
        </div>
        ...
        <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-4 gap-2">
                {['1.5', '2.5', '4.0', '6.0'].map((val) => (
                  <button
                    ...
                    className={`h-12 rounded-lg border text-sm font-bold transition-all duration-300 ${maxPhase === val ? activeClass : inactiveClass}`}
                  >
                    {val}
                  </button>
                ))}
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground mb-2">Проводник прокладывается в трубе, гофре или штробе?</p>
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => setMechProtected('yes')}
                  className={`h-12 rounded-lg border text-sm font-bold transition-all duration-300 ${mechProtected === 'yes' ? activeClass : inactiveClass}`}
                >
                  Да (защищен)
                </button>
                <button
                  onClick={() => setMechProtected('no')}
                  className={`h-12 rounded-lg border text-sm font-bold transition-all duration-300 ${mechProtected === 'no' ? activeClass : inactiveClass}`}
                >
                  Нет (открыто)
                </button>
              </div>
          ...
          <div className="bg-muted/30 rounded-2xl p-6 border border-border flex flex-col justify-center">
            <div className="space-y-6 animate-in zoom-in-95 duration-300">
              ...
              <div className="bg-primary/10 rounded-xl p-5 border border-primary/20 shadow-sm relative overflow-hidden">
                ...
                <p className="text-xs text-primary font-bold uppercase tracking-wider mb-2 relative z-10">Линия: Щит ➔ КУП</p>
                <div className="flex items-baseline gap-2 relative z-10">
                  <span className="text-5xl font-black text-primary">{result.mainWire}</span>
              ...
              <div className="bg-background rounded-xl p-5 border border-border shadow-sm">
                <p className="text-xs text-muted-foreground font-bold uppercase tracking-wider mb-2">Отводы: КУП ➔ Трубы / Ванна</p>
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-3xl font-black text-foreground">{result.branchWire}</span>
                  <span className="text-base font-bold text-muted-foreground">мм²</span>
                </div>
