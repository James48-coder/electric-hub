    ...
    <div className="container mx-auto p-6 max-w-4xl animate-in fade-in duration-500 text-foreground pb-24">
      ...
      <div className="bg-card border border-border rounded-[var(--radius)] shadow-sm overflow-hidden">
        <div className="border-b border-border p-6 bg-primary/5 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">Подбор конденсаторов</h1>
            <p className="text-sm text-muted-foreground mt-1">Расчет емкости для запуска 3Ф двигателя (380В) от сети 220В</p>
          </div>
        </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`h-12 rounded-lg border text-sm font-bold transition-all duration-300 ${connection === 'delta' ? activeClass : inactiveClass}`}
                >
                  Треугольник (Δ)
                </button>
                <button
                  onClick={() => setConnection('star')}
                  className={`h-12 rounded-lg border text-sm font-bold transition-all duration-300 ${connection === 'star' ? activeClass : inactiveClass}`}
                >
                  Звезда (Y)
                </button>
              </div>
          ...
          <div className="bg-muted/30 rounded-2xl p-6 border border-border flex flex-col justify-center">
            {!result ? (
              <div className="text-center text-muted-foreground space-y-3">
                ...
                <p className="text-sm">Введите мощность двигателя для расчета емкостей.</p>
              </div>
            ) : (
              <div className="space-y-6 animate-in zoom-in-95 duration-300">
                ...
                <div className="bg-primary/10 rounded-xl p-5 border border-primary/20 shadow-sm relative overflow-hidden">
                  <div className="absolute -right-4 -bottom-4 opacity-10">
                    <Power className="h-32 w-32 text-primary" />
                  </div>
                  <p className="text-xs text-primary font-bold uppercase tracking-wider mb-2 relative z-10">Рабочий конденсатор (Ср)</p>
                  <div className="flex items-baseline gap-2 relative z-10">
                    <span className="text-5xl font-black text-primary">{result.working}</span>
                    <span className="text-xl font-bold text-primary/70">мкФ</span>
                  </div>
                  <p className="text-sm text-primary/80 mt-1 relative z-10 font-medium">
                    Напряжение не ниже {result.voltage} В
                  </p>
                </div>

                <div className="bg-background rounded-xl p-5 border border-border shadow-sm">
                  <p className="text-xs text-muted-foreground font-bold uppercase tracking-wider mb-2">Пусковой конденсатор (Сп)</p>
                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-black text-foreground">{result.starting}</span>
                    <span className="text-base font-bold text-muted-foreground">мкФ</span>
                  </div>
