...
export function ChatPage() {
  ...
  // Скорректированный и точный интеллектуальный помощник
  const getSmartElectricianResponse = (query: string): string => {
    const q = query.toLowerCase();

    // 1. Сначала проверяем конкретные темы (автоматы, узо, документы), чтобы они не перебивались общими словами
    if (q.includes("автомат") || q.includes("ампер") || q.includes("номинал")) {
    ...
    if (q.includes("пуэ") || q.includes("гост") || q.includes("снип") || q.includes("нормат") || q.includes("документ")) {
    ...
    // 2. Затем проверяем кабели и сечения
    if (q.includes("сечени") || q.includes("кабель") || q.includes("провод")) {
    ...
    // 3. Остальные специфические темы
    ...
    // 4. Резервный сценарий (если ничего из вышеперечисленного не подошло)
