  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 max-w-4xl animate-in fade-in duration-500 text-foreground pb-24">
      ...
      <div className="bg-card border border-border rounded-[var(--radius)] shadow-sm overflow-hidden">
        ...
        <div className="border-b border-border p-4 sm:p-6 bg-yellow-500/5 flex items-start sm:items-center gap-3 sm:gap-4">
          <div className="p-2 sm:p-3 bg-yellow-500/10 rounded-xl text-yellow-500 shrink-0 mt-1 sm:mt-0">
            <TrendingDown className="h-5 w-5 sm:h-6 sm:w-6" />
          </div>
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-foreground leading-tight">Падение напряжения</h1>
            <p className="text-xs sm:text-sm text-muted-foreground mt-1">Расчет потерь в кабеле в зависимости от длины, сечения и нагрузки</p>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          ...
          <div className="space-y-5 sm:space-y-6">
            {/* Поля мощности и длины встают друг под другом на телефонах */}
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              <select
                ...
                className="w-full bg-background border border-border rounded-lg h-10 sm:h-12 px-3 sm:px-4 focus:outline-none focus:ring-2 focus:ring-primary text-sm sm:text-base text-foreground font-medium transition-all cursor-pointer"
              >
          ...
          <div className="bg-muted/30 rounded-2xl p-4 sm:p-6 border border-border flex flex-col justify-center">
            {!result ? (
              <div className="text-center text-muted-foreground space-y-3 py-6 sm:py-0">
                ...
                <p className="text-xs sm:text-sm px-4">Заполните поля мощности, длины и сечения для расчета.</p>
              </div>
            ) : (
              <div className="space-y-4 sm:space-y-6 animate-in zoom-in-95 duration-300">
                <div className={`rounded-xl p-4 sm:p-5 border shadow-sm relative overflow-hidden ${
                  ...
                  <div className="flex justify-between items-start mb-4 relative z-10 gap-2">
                    <div>
                      <p className={`text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-1 ${result.isOk ? 'text-green-600 dark:text-green-400' : 'text-destructive'}`}>
                        Потеря напряжения
                      </p>
                      <div className="flex items-baseline gap-1 sm:gap-2">
                        <span className={`text-4xl sm:text-5xl font-black tracking-tight ${result.isOk ? 'text-green-600 dark:text-green-400' : 'text-destructive'}`}>
                          {result.dropPercent}
                        </span>
                        <span className={`text-lg sm:text-xl font-bold ${result.isOk ? 'text-green-600/70 dark:text-green-400/70' : 'text-destructive/70'}`}>
                          %
                        </span>
                      </div>
                    </div>
                  ...
                  {/* Строка "Абсолютное падение" перестраивается в колонку на узком экране, чтобы не перекрывать друг друга */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between border-t border-border/50 pt-3 sm:pt-4 mt-2 relative z-10 gap-1 sm:gap-0">
                    <span className="text-xs sm:text-sm font-medium text-foreground">Абсолютное падение:</span>
                    <span className="text-sm sm:text-base font-bold text-foreground">{result.dropVolts} Вольт</span>
                  </div>
                </div>

                <div className="bg-background rounded-xl p-3 sm:p-4 border border-border shadow-sm">
                  <div className="flex items-start sm:items-center gap-3">
                    <div className={`p-2 rounded-lg shrink-0 mt-0.5 sm:mt-0 ${result.isOk ? 'bg-green-500/10 text-green-500' : 'bg-destructive/10 text-destructive'}`}>
